...
Note that this work does not make any synchrony assumptions, nor does it discuss liveness. It does not say anything about when validators send or receive messages. It does not address validator rotation, incentive mechanisms, proof-of-stake, light clients, scalability, or message source authentication. The consensus protocols specified here therefore should not be considered completely specified even after the parameters are specified.
...
We will define protocol states ($\Sigma$) as certain sets of messages, and messages ($M$) as certain triples of the form (consensus value, validator name, protocol state). Our target is therefore one that satisfies the following formulas:
...
Note that $F$ is a monotonic function (i.e. $\sigma_1 \subseteq \sigma_2 \implies F(\sigma_1) \leq F(\sigma_2)$). % This should be part of the mathematic notation in Definition 2.11

The CBC Casper protocol states will be parameterized by an equivocation fault tolerance threshold $t$, and will use the following protocol states:
...
We think of protocol-following nodes as existing in these protocol states ($\Sigma_t$), and following the protocol state transitions ($\to$) to get from one state to another. If a node at state $\sigma \in \Sigma_t$ receives messages $m$ such that $\sigma \cup \{m\} \in \Sigma$, then they will transition to state $\sigma \cup \{m\}$, as long as the number of faults is less than the equivocation fault tolerance threshold number (i.e., as long as $F(\sigma \cup \{m\}) \leq t$).
